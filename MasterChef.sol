...
import "./libs/ICookieReferral.sol";
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 0.8 COOKIE per block.
    uint256 public constant INITIAL_EMISSION_RATE = 800 finney;
    // Minimum emission rate: 0.00 COOKIE per block.
    ...
    // Cookie referral contract address.
    ICookieReferral public cookieReferral;
    ...
    constructor(
        ...
        cookiePerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cookieReward = multiplier.mul(cookiePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        cookie.mint(devAddress, cookieReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(cookieReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            cookieReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(cookie)) {
    ...
    function safeCookieTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > cookieBal) {
            transferSuccess = cookie.transfer(_to, cookieBal);
        } else {
            transferSuccess = cookie.transfer(_to, _amount);
        }
        require(transferSuccess, "safeCookieTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(cookiePerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = cookiePerBlock;
        ...
        if (newEmissionRate >= cookiePerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = cookiePerBlock;
        cookiePerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the cookie referral contract address by the owner
    function setCookieReferral(ICookieReferral _cookieReferral) public onlyOwner {
        cookieReferral = _cookieReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(cookieReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = cookieReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                cookie.mint(referrer, commissionAmount);
